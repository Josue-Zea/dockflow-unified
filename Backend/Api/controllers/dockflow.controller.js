...
const createSubDocumento = async (req, res) => {
  const result = await fetchRoute(
    'POST',
    req.body,
    `${SERVER_CONFIG.MS_DOCKFLOW}/dockflow/createSubDocumento`
  );
  res.status(result.code).send(result.data);
}

const getSubDocumentosTypes = async (req, res) => {
  const result = await fetchRoute(
    'GET',
    req.body,
    `${SERVER_CONFIG.MS_DOCKFLOW}/dockflow/getSubDocumentosTypes`
  );
  res.status(result.code).send(result.data);
};

const getSubDocumentosTramite = async (req, res) => {
  const idTramite = req.params.idTramite;
  const result = await fetchRoute(
    'GET',
    req.body,
    `${SERVER_CONFIG.MS_DOCKFLOW}/dockflow/getSubDocumentosTramite/${idTramite}`
  );
  res.status(result.code).send(result.data);
};
...
const getSubdocumentoPDF = async (req, res) => {
  ...
  const result = await fetchRoute(
    'GET',
    req.body,
    `${SERVER_CONFIG.MS_DOCKFLOW}/dockflow/getSubDocumento/${idSubDocumento}`
  );
  res.status(result.code).send(result.data);
};
...
const deleteSubDocumento = async (req, res) => {
  ...
  const result = await fetchRoute(
    'DELETE',
    null,
    `${SERVER_CONFIG.MS_DOCKFLOW}/dockflow/deleteSubDocumento/${idSubDocumento}/${idDocumento}`
  );
  res.status(result.code).send(result.data);
}
...
module.exports = {
  ...
  deleteTramite,
  createSubDocumento,
  getSubDocumentosTypes,
  getSubDocumentosTramite,
  getSubdocumentoPDF,
  deleteSubDocumento
};